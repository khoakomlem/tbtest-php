<?php
    ...
    if (isset($_GET['code'])){
    	...
    	$txt = "SELECT * FROM `ha_member` WHERE `recovery_auth` = '%s' LIMIT 1";
    	...
    	if (mysqli_num_rows($result) == 1) {
    		...
    		if (isset($_POST['changepass'])){
    			...
    			$txt = "UPDATE `ha_member` SET `recovery_auth`='', `password`='%s' WHERE `recovery_auth`='%s'";
    ...
    if (isset($_POST['xacnhan']) && isset($_POST['username'])){
    	...
    	$result = mysqli_query($db, "SELECT * FROM `ha_member` WHERE `username`='".$username."' LIMIT 1");
    	if (mysqli_num_rows($result) == 1){
    		$t = mysqli_fetch_assoc($result);
    		$email = $t['email'];
    		$name = $t['name'];
    		$code = strtoupper(md5(uniqid(rand())));
    		mysqli_query($db, "UPDATE `ha_member` SET `recovery_auth`='".$code."' WHERE `username` = '".$username."' LIMIT 1");
    		...
    		$body = sprintf($file, $username, 'localhost/hatest/recovery.php?code='.$code, $email);
    		sendMail($email, $body);	
    	} else {
