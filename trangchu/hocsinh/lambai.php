...
$id = strtoupper(mysqli_fetch_assoc(mysqli_query($db, "SELECT `test` FROM `member` WHERE `username` = '".$_SESSION['username']."' LIMIT 1"))['test']);
if ($id == ''){
...
if (mysqli_num_rows($result) == 1){
    ...
    if (isset($_POST['nopbailuon'])){
        ...
        //
        $txt = "UPDATE `member` SET `test`='', `thongtin`='%s' WHERE `username`='".$_SESSION['username']."'";
        $sql = sprintf($txt, addslashes(json_encode($result)));
        mysqli_query($db, $sql);
        //
    ...
    if ($_COOKIE['num']+1 > count($t)){
        $thongtin = json_decode(mysqli_fetch_assoc(mysqli_query($db, "SELECT `thongtin` FROM `member` WHERE username='".$_SESSION['username']."'"))['thongtin'], true);
        ...
        mysqli_query($db, "UPDATE `member` SET `test` ='', `thongtin`='".$thongtin."' WHERE `username` = '".$_SESSION['username']."'");
        setcookie('num', NULL, -1);
        header("Location: ketqua.php?id=".$id);
        die();
    } else
    ...
    if ($now>$tmp['done_at']){
        setcookie('num', NULL, -1);
        mysqli_query($db, "UPDATE `member` SET `test` ='' WHERE `username` = '".$_SESSION['username']."'");
        header("Location: ketqua.php?id=".$id);
        die();
    }
}
