...
if (isset($_POST['signup'])){
    ...
    if ($ok){
        ...
        //check trùng user
        $txt = "SELECT * FROM `member` WHERE `username`='%s' OR `email`='%s'";
        ...
        if (mysqli_num_rows($results) == 1) {
            ...
            $txt = "INSERT INTO `member` (`id`, `name`, `username`, `password`, `email`, `type`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')";
            ...
            mysqli_query($db, "UPDATE `member` SET `email_auth`='".$code."' WHERE `username` = '".$username."' LIMIT 1");

            $file = implode("<br>", explode("\n", file_get_contents('../mailer/email_auth.txt')));
            $body = sprintf($file, $username, 'http://tbtest.byethost12.com/email_auth.php?code='.$code, $email);
            sendMail($email, $body);
        }
        //
