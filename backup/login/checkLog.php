<?php
    ...
    if (isset($_SESSION['username'])) {
        $txt = "SELECT `type` FROM `ha_member` WHERE `username`='%s' LIMIT 1";
    ...
    if (isset($_POST['login'])){
        ...
        if ($ok){
            ...
            $txt = "SELECT * FROM `ha_member` WHERE `username`='%s' AND `password`='%s' LIMIT 1";
            $sql = sprintf($txt, $username, $password);
            $results = mysqli_query($db, $sql);
            if (mysqli_num_rows($results) == 1) { // login thanh cong
                $t = mysqli_fetch_assoc($results);
                if ($t['email_auth'] != ''){
                    echo "Bạn chưa xác thực gmail của bạn! Bấm <a href='../index.php'>vào đây</a> để quay về trang chủ";
                    exit();
                }
